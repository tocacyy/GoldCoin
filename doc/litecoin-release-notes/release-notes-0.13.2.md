Eurocoin Core version 0.13.2 is now available from:

  <https://download.eurocoin.org/eurocoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/eurocoin-project/eurocoin/issues>
...
updates will be released anymore. Without security updates, using a eurocoin
...
ECDSA signatures inside Eurocoin transactions now use validation using
...
Previous versions of Eurocoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Eurocoin Core was to raise the default minimum
relay fee.

Eurocoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Eurocoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running eurocoind without having to do any manual configuration.
...
Eurocoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Eurocoin Core will relay transactions with
...
entirely in Eurocoin Core 0.13, and it is at this time undecided whether the
...
Eurocoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Eurocoin Core automatically creates a hidden service to listen on, without
manual configuration. Eurocoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Eurocoin Core is listening, and
...
Eurocoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Eurocoin
Core to use floating fees. By default, Eurocoin Core will use floating
fees.
...
(default: `0.0002` EUC/kB).

At all times, Eurocoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) EUC.
Furthermore, Eurocoin Core will never create transactions paying less than
...
`eurocoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 212043 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 212043 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /eurocoinrpc>
        ProxyPass http://127.0.0.1:12043/
        ProxyPassReverse http://127.0.0.1:12043/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass eurocoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< eurocoinrpc:<password>
    </Location>
...
Restarting eurocoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in eurocoin.conf
...
eurocoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/eurocoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Eurocoin Core code base has
...
- `eurocoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `eurocoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Eurocoin scripting system that in combination with
...
Eurocoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Eurocoin Core.
...
The two phases are distinct in the Eurocoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Eurocoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Eurocoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Eurocoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of eurocoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of eurocoins being spent is exactly
...
- **Increased security for multisig:** Eurocoin addresses (both P2PKH addresses
  ...
  believed to be provided by Eurocoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Eurocoin's current ledger (called the UTXO set),
  ...
  note that Eurocoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Eurocoin users to individually opt-in to almost any change in the Eurocoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Eurocoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Eurocoin
client to Eurocoin Core 0.13.2 to signal for segwit support. When a
...
Since Eurocoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `eurocoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `eurocoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
